...
class LocalSaveManager {
  ...
  async createEphemeralBackup(objectId, shop, _downloadOptionTitle, _label) {
    const tempRoot = await fs__default.promises.mkdtemp(path__default.join(os$1.tmpdir(), "cloud-save-ephemeral-"));
    const normalizedObjectId = this.normalizeGameName(objectId);
    const backupId = crypto.randomUUID();
...
class BackupManager {
   ...
   */
  getSecureAppDataPath(subDir) {
    try {
      const userDataPath = app.getPath("userData");
      const fullPath = path$1.join(userDataPath, subDir);
      return fullPath;
    } catch (error) {
      console.error("❌ Erro ao obter caminho seguro do userData:", error);
      const tempPath = path$1.join(os.tmpdir(), "project-w", subDir);
      return tempPath;
    }
  }
  /**
    ...
    try {
      ...
      if (!backupPath || backupPath.trim() === "") {
        ...
        if (!fs$1.existsSync(fallbackPath)) {
          fs$1.mkdirSync(fallbackPath, { recursive: true });
        }
        return;
      }
      if (!fs$1.existsSync(backupPath)) {
      ...
      const configPath = path$1.join(backupPath, "config");
      const cloudSavesPath = path$1.join(backupPath, "CloudSaves");
      if (!fs$1.existsSync(configPath)) {
        fs$1.mkdirSync(configPath, { recursive: true });
      }
      if (!fs$1.existsSync(cloudSavesPath)) {
        fs$1.mkdirSync(cloudSavesPath, { recursive: true });
      }
    } catch (error) {
...
async function checkAndDownloadLudusavi() {
  try {
    ...
    if (!fs$1.existsSync(ludusaviDir)) {
      fs$1.mkdirSync(ludusaviDir, { recursive: true });
    }
    ...
    const tempPath = path$1.join(ludusaviDir, assetName);
    const writer = fs$1.createWriteStream(tempPath);
    ...
    if (assetName.endsWith(".zip")) {
      ...
      const extractedBinary = path$1.join(ludusaviDir, "ludusavi.exe");
      if (fs$1.existsSync(extractedBinary)) {
        fs$1.renameSync(extractedBinary, Ludusavi.getBinaryPath());
      }
      ...
      if (fs$1.existsSync(Ludusavi.getBinaryPath())) {
        fs$1.chmodSync(Ludusavi.getBinaryPath(), 493);
      }
    }
